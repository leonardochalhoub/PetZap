...
function buildSystemInstruction(petNames: string[]): string {
  ...
  return [
    ...
    "- pesou, pesando, pesagem, está com X kg, tem X kg, X quilos, X kg → intent=weight (uses weight_kg, measured_at; accept comma or dot decimal: '8,5kg' → 8.5). ANY mention of a weight number for a pet counts as this intent, even if no verb is present ('Poli 9kg hoje').",
    ...
    "",
    '13. Input: "Poli está pesando 9kg" (or transcript of a voice note)',
    `    Output: {"intents":[{"intent":"weight","pet_name":"Poli","weight_kg":9,"measured_at":"${today}"}]}`,
    "",
    '14. Input: "Dora 7,3 quilos hoje"',
    `    Output: {"intents":[{"intent":"weight","pet_name":"Dora","weight_kg":7.3,"measured_at":"${today}"}]}`,
    "",
